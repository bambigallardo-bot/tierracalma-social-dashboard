...
async function getMetaAds(months) {
  ...
  for (const r of rows) {
    ...
    if (!key) continue;
    if (!isIncluded(r.campaign_name)) continue; // solo campañas de Tierra Calma
  ...
  const monthly = months.map((m) => {
    ...
    return {
      ...
      cpc: sum(camps, "clicks") ? round(spend / sum(camps, "clicks")) : null,
...
async function getBestAds(months) {
  ...
  for (const [key, list] of byMonth.entries()) {
    ...
    topByMonth[key] = list.slice(0, TOP);
  }

  // Miniaturas: solo para los meses MÁS RECIENTES (los que se ven en el informe),
  // y en paralelo por lotes — así no se dispara el timeout de 60s.
  const recentKeys = Object.keys(topByMonth).sort().slice(-6); // últimos 6 meses
  const wantIds = [];
  for (const key of recentKeys) {
    for (const a of topByMonth[key]) if (a.adId && !wantIds.includes(a.adId)) wantIds.push(a.adId);
  }
  const capped = wantIds.slice(0, 30);
  const thumbById = {};
  const CHUNK = 6;
...
export async function getMetaDashboard() {
  ...
  // En PARALELO para no pasarnos del límite de 60s (el tiempo total = la consulta
  // más lenta, no la suma). Tierra Calma: Instagram orgánico + Meta Ads (solo campañas
  // de Tierra Calma) + mejores anuncios a nivel de anuncio (con imagen del creativo).
  const [ig, ads, bestAds] = await Promise.all([
    settle(() => getInstagram(months)),
    settle(() => getMetaAds(months)),
    settle(() => getBestAds(months)),
  ]);
